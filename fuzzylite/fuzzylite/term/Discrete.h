...
namespace fl {
    ...
    class FL_API Discrete : public Term {
      ...
      public:
        ...
        /**
          Computes the membership function evaluated at @f$x@f$ by using
          binary search to find the lower and upper bounds of @f$x@f$ and
          then linearly interpolating the membership function between the
          bounds.
          @param x
          @return @f$ \dfrac{h (y_{\max} - y_{\min})}{(x_{\max}- x_{\min})}
          (x
          - x_{\min}) + y_{\min}@f$
          ...
          limits of @f$x@f$ in `xy` (respectively),
                @f$y_{\min}@f$ and @f$y_{\max}@f$is are the membership
          functions of @f$\mu(x_{\min})@f$ and @f$\mu(x_{\max})@f$
          (respectively)
         */
        ...
        /**
          ...
          @param pairs is the vector of pairs defining the discrete
          membership function
         */
        ...
        /**
          ...
          function
          @return an immutable vector of pairs defining the discrete
          membership function
         */
        ...
        /**
          ...
          @throws fl::Exception if a value is missing, that is, if the
          length of @f$xy@f$ is odd: @f$|xy|\mod 2 = 1@f$
         */
        ...
        /**
          ...
          @param missingValue is the replacement in the case a value is
          missing from @f$xy@f$
          @return a vector of Pair in the form
          @f$\left(\{x_1,y_1\},...,\{x_n,y_n\}\right)@f$
         */
        ...
        /**
          ...
          @param suffix indicates the postfix of a Pair, e.g., `]` results
          in
          @f$y_i]@f$
          @param outerSeparator indicates the separator between Pair, e.g.,
          `;` results in @f$(x_i,y_i);(x_j,y_j)@f$
          @return a formatted string containing the pairs of @f$(x,y)@f$
          values
         */
        ...
        /**
          ...
          @f$(1.0, 2, 3.0)@f$ the second term is an integer, which will
          cause memory access issues due to the difference in size between
          `int` and `fl::scalar`.
          ...
          @param ... are the remaining pairs of values @f$x_i@f$ and
          @f$y_i@f$
