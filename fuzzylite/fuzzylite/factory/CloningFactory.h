...
namespace fl {
    ...
    class CloningFactory {
       ...
       public:
        ...
        /**
          Creates a cloned object by executing the clone method on the
          registered object
          @param key is the unique name by which objects are registered
          @return a cloned object by executing the clone method on the
          registered object
         */
...
namespace fl {
    ...
    template <typename T>
    inline CloningFactory<T>& CloningFactory<T>::operator=(
        const CloningFactory& other) {
        if (this != &other) {
            typename std::map<std::string, T>::const_iterator it
                = this->_objects.begin();
    ...
    template <typename T>
    inline void CloningFactory<T>::registerObject(const std::string& key,
                                                  T object) {
    ...
    inline T CloningFactory<T>::cloneObject(const std::string& key) const {
        ...
        throw Exception("[cloning error] " + _name + " object by name <" + key
                            + "> not registered",
                        FL_AT);
    }
