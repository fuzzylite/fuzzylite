...
std::vector<Console::Option> Console::availableOptions() {
  ...
  options.push_back(
      Option(KW_INPUT_FILE, "inputfile", "file to import your engine from"));
  options.push_back(Option(KW_INPUT_FORMAT,
                           "format",
                           "format of the file to import (fll | fis | fcl)"));
  options.push_back(
      Option(KW_OUTPUT_FILE, "outputfile", "file to export your engine to"));
...
std::map<std::string, std::string> Console::parse(int argc,
  ...
  if ((argc - 1) % 2 != 0) {
    throw Exception(
        "[option error] incomplete number of parameters [key value]", FL_AT);
  }
  ...
  if (options.size() == 1) {
    ...
    for (std::map<std::string, std::string>::const_iterator it
      ...
      if (not isValid) {
        throw Exception(
            "[option error] option <" + it->first + "> not recognized", FL_AT);
      }
    }
  }
  return options;
...
void Console::process(const std::map<std::string, std::string>& options) {
  ...
  if (isExample) {
    ...
    if (example == "m" or example == "mamdani") {
      ...
      throw Exception("[option error] example <" + example + "> not available",
                      FL_AT);
    }
    inputFormat = "fll";
    ...
    if (not inputFile.is_open()) {
      throw Exception(
          "[file error] file <" + inputFilename + "> could not be opened",
          FL_AT);
    }
    std::string line;
    ...
    if (it != options.end()) {
      ...
      if (extensionIndex != std::string::npos) {
        ...
        throw Exception("[format error] unspecified format of input file",
                        FL_AT);
      }
    }
  }
  ...
  if (it != options.end()) {
    ...
    if (extensionIndex != std::string::npos) {
      ...
      throw Exception("[format error] unspecified format of output file",
                      FL_AT);
    }
  }
  ...
  if (outputFilename.empty()) {
    ...
    if (not writer.is_open()) {
      throw Exception(
          "[file error] file <" + outputFilename + "> could not be created",
          FL_AT);
    }
  ...
  if ("fld" == outputFormat) {
    ...
    if ((it = options.find(KW_DATA_INPUT_FILE)) != options.end()) {
      ...
      if (not dataFile.is_open()) {
        throw Exception(
            "[export error] file <" + it->second + "> could not be opened",
            FL_AT);
      }
      try {
      ...
      if ((it = options.find(KW_DATA_VALUES)) != options.end()) {
        ...
        if ((it = options.find(KW_DATA_VALUES_SCOPE)) != options.end()) {
          if ("AllVariables" == it->second)
            ...
            throw Exception(
                "[export error] unknown scope of values <" + it->second + ">",
                FL_AT);
        }
...
Engine* Console::takagiSugeno() {
  ...
  ruleBlock->addRule(Rule::parse(
      "if inputX is any then trueValue is fx and difference is error", engine));
  engine->addRuleBlock(ruleBlock);
...
Engine* Console::hybrid() {
  ...
  mamdaniRuleBlock->addRule(
      Rule::parse("if service is excellent or food is delicious then mTip is "
                  "generous with 0.5",
                  engine));
  mamdaniRuleBlock->addRule(
      Rule::parse("if service is excellent and food is delicious then mTip is "
                  "generous with 1.0",
                  engine));
  ...
  takagiSugenoRuleBlock->addRule(
      Rule::parse("if service is excellent or food is delicious then tsTip is "
                  "generous with 0.5",
                  engine));
  takagiSugenoRuleBlock->addRule(
      Rule::parse("if service is excellent and food is delicious then tsTip is "
                  "generous with 1.0",
                  engine));
...
void Console::exportAllExamples(const std::string& from,
  ...
  if (from == "fll")
    ...
    throw Exception(
        "[examples error] unrecognized format <" + from + "> to import", FL_AT);
  ...
  if (to == "fll")
    ...
    throw Exception(
        "[examples error] unrecognized format <" + to + "> to export", FL_AT);
  ...
  for (std::size_t i = 0; i < examples.size(); ++i) {
    ...
    if (target.is_open()) {
      if (to == "cpp") {
        ...
        InputVariable* b
            = engine->getInputVariable(1 % engine->numberOfInputVariables());
        std::string pathToDF
            = example.substr(example.find_last_of('/') + 1) + ".fld";
