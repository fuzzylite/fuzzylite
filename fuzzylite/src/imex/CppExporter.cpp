...
namespace fl {
    ...
    std::string CppExporter::toString(const InputVariable* inputVariable,
        ...
        if (isUsingVariableNames()) {
            ...
            if (engine->numberOfInputVariables() > 1) {
                std::size_t index
                    = std::distance(engine->inputVariables().begin(),
    ...
    std::string CppExporter::toString(const OutputVariable* outputVariable,
                                      const Engine* engine) const {
        std::string name;
        if (isUsingVariableNames()) {
            ...
            if (engine->numberOfOutputVariables() > 1) {
                std::size_t index
                    = std::distance(engine->outputVariables().begin(),
                                    std::find(engine->outputVariables().begin(),
                                              engine->outputVariables().end(),
                                              outputVariable));
                name += Op::str(index + 1);
            }
        }
        std::ostringstream ss;
        ss << fl("OutputVariable* ") << name << " = new "
           << fl("OutputVariable;\n");
    ...
    std::string CppExporter::toString(const RuleBlock* ruleBlock,
                                      const Engine* engine) const {
        std::string name;
        ...
        if (isUsingVariableNames() and not ruleBlock->getName().empty()) {
            ...
            if (engine->numberOfRuleBlocks() > 1) {
                std::size_t index
                    = std::distance(engine->ruleBlocks().begin(),
                                    std::find(engine->ruleBlocks().begin(),
                                              engine->ruleBlocks().end(),
                                              ruleBlock));
                name += Op::str(index + 1);
            }
        }
    ...
    std::string CppExporter::toString(const Activation* activation) const {
        ...
        for (std::size_t i = 0; i < values.size(); ++i) {
            ...
            values.at(i)
                = (Op::isNumeric(parameter) ? parameter
                                            : ("\"" + parameter + "\""));
        }
        return "new " + fl(activation->className()) + "("
               + Op::join(values, ", ") + ")";
