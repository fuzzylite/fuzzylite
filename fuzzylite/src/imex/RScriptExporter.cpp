...
namespace fl {
    ...
    std::string RScriptExporter::toString(const Engine* engine) const {
        if (engine->inputVariables().empty()) {
            throw Exception(
                "[exporter error] engine has no input variables to export the "
                "surface",
                FL_AT);
        }
        if (engine->outputVariables().empty()) {
            throw Exception(
                "[exporter error] engine has no output variables to export the "
                "surface",
                FL_AT);
        }
        InputVariable* a = engine->inputVariables().at(0);
        InputVariable* b
            = engine->inputVariables().at(1 % engine->numberOfInputVariables());
        return toString(const_cast<Engine*>(engine),
                        a,
                        b,
                        1024,
                        FldExporter::AllVariables,
    ...
    std::string RScriptExporter::toString(
        ...
        writeScriptExportingDataFrame(
            engine, writer, a, b, values, scope, outputVariables);
        return writer.str();
    }
    ...
    std::string RScriptExporter::toString(
        ...
        std::ostringstream writer;
        writeScriptExportingDataFrame(
            engine, writer, a, b, reader, outputVariables);
        return writer.str();
    }

    void RScriptExporter::toFile(const std::string& filePath,
    ...
    void RScriptExporter::toFile(
        ...
        if (not writer.is_open()) {
            throw Exception(
                "[file error] file <" + filePath + "> could not be created",
                FL_AT);
        }
        writeScriptExportingDataFrame(
            engine, writer, a, b, values, scope, outputVariables);
    ...
    void RScriptExporter::writeScriptImportingDataFrame(
        ...
        writer << "if (require(data.table)) {\n"
               << "    engine.df = data.table::fread(engine.fldFile, "
    ...
    void RScriptExporter::writeScriptExportingDataFrame(
        ...
        writer << "\"\n\n";
        writer << "engine.df = read.delim(textConnection(engine.fld), "
                  "header=TRUE, "
    ...
    void RScriptExporter::writeScriptHeader(std::ostream& writer,
                                            const Engine* engine) const {
    ...
    void RScriptExporter::writeScriptPlots(
        ...
        for (std::size_t i = 0; i < outputVariables.size(); ++i) {
            ...
            if (a != b) {
                writer << "engine.plot.i1i2_o" << (i + 1)
                       << " = ggplot(engine.df, aes(" << a->getName() << ", "
                       << b->getName() << ")) + \n"
                       << "    geom_tile(aes(fill=" << z->getName() << ")) + \n"
                       << "    scale_fill_gradient(low=\"" << _minimumColor
                       << "\", high=\"" << _maximumColor << "\") + \n"
                       << "    stat_contour(aes(x=" << a->getName()
                       << ", y=" << b->getName() << ", z=" << z->getName()
                       << "), color=\"" << _contourColor << "\") + \n"
                       << "    ggtitle(\"(" << a->getName() << ", "
                       << b->getName() << ") = " << z->getName() << "\")\n\n";

                writer << "engine.plot.i2i1_o" << (i + 1)
                       << " = ggplot(engine.df, aes(" << b->getName() << ", "
                       << a->getName() << ")) + \n"
                       << "    geom_tile(aes(fill=" << z->getName() << ")) + \n"
                       << "    scale_fill_gradient(low=\"" << _minimumColor
                       << "\", high=\"" << _maximumColor << "\") + \n"
                       << "    stat_contour(aes(x=" << b->getName()
                       << ", y=" << a->getName() << ", z=" << z->getName()
                       << "), color=\"" << _contourColor << "\") + \n"
                       << "    ggtitle(\"(" << b->getName() << ", "
                       << a->getName() << ") = " << z->getName() << "\")\n\n";
                ...
                writer
                    << "engine.plot.i1_o" << (i + 1)
                    << " = ggplot(engine.df, aes(" << a->getName() << ", "
                    << z->getName() << ")) + \n"
                    << "    geom_line(aes(color=" << z->getName()
                    << "), size=3, lineend=\"round\", linejoin=\"mitre\") + \n"
                    << "    scale_color_gradient(low=\"" << _minimumColor
                    << "\", high=\"" << _maximumColor << "\") + \n"
                    << "    ggtitle(\"" << a->getName() << " vs "
                    << z->getName() << "\")\n\n";

                writer
                    << "engine.plot.o" << (i + 1)
                    << "_i1 = ggplot(engine.df, aes(" << a->getName() << ", "
                    << z->getName() << ")) + \n"
                    << "    geom_line(aes(color=" << z->getName()
                    << "), size=3, lineend=\"round\", linejoin=\"mitre\") + \n"
                    << "    scale_color_gradient(low=\"" << _minimumColor
