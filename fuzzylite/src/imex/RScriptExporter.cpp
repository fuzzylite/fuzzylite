...
std::string RScriptExporter::toString(const Engine* engine) const {
  if (engine->inputVariables().empty()) {
    throw Exception(
        "[exporter error] engine has no input variables to export the surface",
        FL_AT);
  }
  if (engine->outputVariables().empty()) {
    throw Exception(
        "[exporter error] engine has no output variables to export the surface",
        FL_AT);
  }
...
void RScriptExporter::toFile(
  ...
  if (not writer.is_open()) {
    throw Exception("[file error] file <" + filePath + "> could not be created",
                    FL_AT);
  }
  writeScriptExportingDataFrame(
      engine, writer, a, b, values, scope, outputVariables);
...
void RScriptExporter::writeScriptImportingDataFrame(
  ...
  writer << "if (require(data.table)) {\n"
         << "    engine.df = data.table::fread(engine.fldFile, sep=\"auto\", "
            "header=\"auto\")\n"
         << "} else {\n"
...
void RScriptExporter::writeScriptPlots(
  ...
  for (std::size_t i = 0; i < outputVariables.size(); ++i) {
    ...
    if (a != b) {
      writer << "engine.plot.i1i2_o" << (i + 1) << " = ggplot(engine.df, aes("
             << a->getName() << ", " << b->getName() << ")) + \n"
             ...
             << "), color=\"" << _contourColor << "\") + \n"
             << "    ggtitle(\"(" << a->getName() << ", " << b->getName()
             << ") = " << z->getName() << "\")\n\n";

      writer << "engine.plot.i2i1_o" << (i + 1) << " = ggplot(engine.df, aes("
             << b->getName() << ", " << a->getName() << ")) + \n"
             << "    geom_tile(aes(fill=" << z->getName() << ")) + \n"
             << "    scale_fill_gradient(low=\"" << _minimumColor
             << "\", high=\"" << _maximumColor << "\") + \n"
             ...
             << "), color=\"" << _contourColor << "\") + \n"
             << "    ggtitle(\"(" << b->getName() << ", " << a->getName()
             << ") = " << z->getName() << "\")\n\n";
      arrangeGrob << "engine.plot.i1i2_o" << (i + 1) << ", "
                  << "engine.plot.i2i1_o" << (i + 1) << ", ";
    } else {
      writer << "engine.plot.i1_o" << (i + 1) << " = ggplot(engine.df, aes("
             << a->getName() << ", " << z->getName() << ")) + \n"
      ...
      writer << "engine.plot.o" << (i + 1) << "_i1 = ggplot(engine.df, aes("
             << a->getName() << ", " << z->getName() << ")) + \n"
             << "    geom_line(aes(color=" << z->getName()
             << "), size=3, lineend=\"round\", linejoin=\"mitre\") + \n"
             << "    scale_color_gradient(low=\"" << _minimumColor
