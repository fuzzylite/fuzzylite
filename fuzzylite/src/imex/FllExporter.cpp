...
std::string FllExporter::toString(
...
std::string FllExporter::toString(
    const std::vector<InputVariable*>& variables) const {
  std::vector<std::string> result;
  for (std::size_t i = 0; i < variables.size(); ++i) {
    result.push_back(toString(variables.at(i)));
  }
...
std::string FllExporter::toString(
    const std::vector<OutputVariable*>& variables) const {
  std::vector<std::string> result;
  for (std::size_t i = 0; i < variables.size(); ++i) {
    result.push_back(toString(variables.at(i)));
  }
...
std::string FllExporter::toString(const Variable* variable) const {
  ...
  result.push_back(_indent
                   + "enabled: " + (variable->isEnabled() ? "true" : "false"));
  result.push_back(
      _indent + "range: "
      + Op::join(2, " ", variable->getMinimum(), variable->getMaximum()));
...
std::string FllExporter::toString(const InputVariable* inputVariable) const {
  ...
  if (not inputVariable->getDescription().empty()) {
    result.push_back(_indent
                     + "description: " + inputVariable->getDescription());
  }
  result.push_back(
      _indent + "enabled: " + (inputVariable->isEnabled() ? "true" : "false"));
  result.push_back(
      _indent + "range: "
      + Op::join(
          2, " ", inputVariable->getMinimum(), inputVariable->getMaximum()));
  result.push_back(_indent + "lock-range: "
...
std::string FllExporter::toString(const OutputVariable* outputVariable) const {
  std::vector<std::string> result;
  result.push_back("OutputVariable: "
                   + Op::validName(outputVariable->getName()));
  if (not outputVariable->getDescription().empty()) {
    result.push_back(_indent
                     + "description: " + outputVariable->getDescription());
  }
  result.push_back(
      _indent + "enabled: " + (outputVariable->isEnabled() ? "true" : "false"));
  result.push_back(
      _indent + "range: "
      + Op::join(
          2, " ", outputVariable->getMinimum(), outputVariable->getMaximum()));
  result.push_back(_indent + "lock-range: "
  ...
  result.push_back(
      _indent + "defuzzifier: " + toString(outputVariable->getDefuzzifier()));
  result.push_back(_indent
                   + "default: " + Op::str(outputVariable->getDefaultValue()));
  result.push_back(
      _indent + "lock-previous: "
...
std::string FllExporter::toString(const RuleBlock* ruleBlock) const {
  ...
  result.push_back(_indent
                   + "enabled: " + (ruleBlock->isEnabled() ? "true" : "false"));
  result.push_back(_indent
                   + "conjunction: " + toString(ruleBlock->getConjunction()));
  result.push_back(_indent
                   + "disjunction: " + toString(ruleBlock->getDisjunction()));
  result.push_back(_indent
                   + "implication: " + toString(ruleBlock->getImplication()));
  result.push_back(_indent
                   + "activation: " + toString(ruleBlock->getActivation()));
