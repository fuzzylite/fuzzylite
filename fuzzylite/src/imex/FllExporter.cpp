...
namespace fl {
    ...
    std::string FllExporter::toString(const Variable* variable) const {
        ...
        result.push_back(
            _indent + "enabled: " + (variable->isEnabled() ? "true" : "false"));
        result.push_back(
            _indent + "range: "
    ...
    std::string FllExporter::toString(
        ...
        std::vector<std::string> result;
        result.push_back("InputVariable: "
                         + Op::validName(inputVariable->getName()));
        if (not inputVariable->getDescription().empty()) {
            result.push_back(
                _indent + "description: " + inputVariable->getDescription());
        }
        result.push_back(_indent + "enabled: "
                         + (inputVariable->isEnabled() ? "true" : "false"));
        result.push_back(_indent + "range: "
                         + Op::join(2,
                                    " ",
    ...
    std::string FllExporter::toString(
        ...
        if (not outputVariable->getDescription().empty()) {
            result.push_back(
                _indent + "description: " + outputVariable->getDescription());
        }
        result.push_back(_indent + "enabled: "
        ...
        result.push_back(
            _indent + "default: " + Op::str(outputVariable->getDefaultValue()));
        result.push_back(
    ...
    std::string FllExporter::toString(const RuleBlock* ruleBlock) const {
        ...
        if (not ruleBlock->getDescription().empty()) {
            result.push_back(_indent
                             + "description: " + ruleBlock->getDescription());
        }
        result.push_back(_indent + "enabled: "
                         + (ruleBlock->isEnabled() ? "true" : "false"));
        result.push_back(
            _indent + "conjunction: " + toString(ruleBlock->getConjunction()));
        result.push_back(
            _indent + "disjunction: " + toString(ruleBlock->getDisjunction()));
        result.push_back(
            _indent + "implication: " + toString(ruleBlock->getImplication()));
        result.push_back(
            _indent + "activation: " + toString(ruleBlock->getActivation()));
    ...
    std::string FllExporter::toString(const Term* term) const {
        return "term: " + Op::validName(term->getName()) + " "
               + term->className() + " " + term->parameters();
    }
