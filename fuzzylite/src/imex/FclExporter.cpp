...
namespace fl {
    ...
    std::string FclExporter::toString(const Engine* engine) const {
        ...
        for (std::size_t i = 0; i < engine->numberOfInputVariables(); ++i) {
            fcl << _indent
                << Op::validName(engine->getInputVariable(i)->getName())
                << ": REAL;\n";
        }
        fcl << "END_VAR\n\n";
        ...
        for (std::size_t i = 0; i < engine->numberOfOutputVariables(); ++i) {
            fcl << _indent
                << Op::validName(engine->getOutputVariable(i)->getName())
                << ": REAL;\n";
        }
        fcl << "END_VAR\n\n";
    ...
    std::string FclExporter::toString(
        ...
        for (std::size_t t = 0; t < inputVariable->numberOfTerms(); ++t) {
            ...
            fcl << _indent << "TERM " << Op::validName(term->getName())
                << " := " << toString(term) << ";\n";
        }
        fcl << "END_FUZZIFY\n";
        return fcl.str();
    ...
    std::string FclExporter::toString(
        ...
        if (outputVariable->getDefuzzifier()) {
            fcl << _indent
                << "METHOD : " << toString(outputVariable->getDefuzzifier())
                << ";\n";
        }
