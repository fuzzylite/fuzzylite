...
Engine* FisImporter::fromString(const std::string& fis) const {
  ...
  for (std::size_t i = 0; i < sections.size(); ++i) {
    if ("[System]" == sections.at(i).substr(0, std::string("[System]").size()))
      ...
      throw Exception(
          "[import error] section <" + sections.at(i) + "> not recognized",
          FL_AT);
  }
  ...
  while (std::getline(reader, line)) {
    ...
    if (key == "Name")
      ...
      defuzzMethod = value;
             ...
             or key == "NumOutputs" or key == "NumRules" or key == "NumMFs") {
      // ignore because are redundant.
    } else
      throw Exception("[import error] token <" + key + "> not recognized",
                      FL_AT);
  }
}
    ...
    for (std::size_t i = 0; i < antecedent.size(); ++i) {
      ...
      if (i + 1 < antecedent.size()) {
        ...
        if (connector == "1")
          ...
          throw Exception(
              "[syntax error] connector <" + connector + "> not recognized",
              FL_AT);
      }
    }
...
Term* FisImporter::parseTerm(const std::string& fis,
  ...
  return createInstance(
      Op::trim(termParams.at(0)), Op::trim(nameTerm.at(0)), parameters, engine);
}
