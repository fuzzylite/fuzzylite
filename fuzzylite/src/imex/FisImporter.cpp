...
namespace fl {
    ...
    Engine* FisImporter::fromString(const std::string& fis) const {
        ...
        for (std::size_t i = 0; i < sections.size(); ++i) {
            if ("[System]"
                == sections.at(i).substr(0, std::string("[System]").size()))
                importSystem(sections.at(i),
                             engine.get(),
                             andMethod,
                             orMethod,
                             impMethod,
                     ...
                     == sections.at(i).substr(0, std::string("[Input").size()))
                importInput(sections.at(i), engine.get());
            else if ("[Output"
                     == sections.at(i).substr(0, std::string("[Output").size()))
                importOutput(sections.at(i), engine.get());
            else if ("[Rules]"
                     == sections.at(i).substr(0, std::string("[Rules]").size()))
                importRules(sections.at(i), engine.get());
            else
                throw Exception("[import error] section <" + sections.at(i)
                                    + "> not recognized",
                                FL_AT);
        }
        engine->configure(translateTNorm(andMethod),
    ...
    void FisImporter::importSystem(const std::string& section,
                                   Engine* engine,
        ...
        while (std::getline(reader, line)) {
            ...
            if (key == "Name")
                ...
                defuzzMethod = value;
            else if (key == "Type" or key == "Version" or key == "NumInputs"
                     or key == "NumOutputs" or key == "NumRules"
                     or key == "NumMFs") {
                // ignore because are redundant.
            } else
                throw Exception(
                    "[import error] token <" + key + "> not recognized", FL_AT);
        }
    }

    void FisImporter::importInput(const std::string& section,
                                  Engine* engine) const {
        std::istringstream reader(section);
        std::string line;
    ...
    void FisImporter::importOutput(const std::string& section,
                                   Engine* engine) const {
        std::istringstream reader(section);
        std::string line;
        ...
        while (std::getline(reader, line)) {
            ...
            if (key == "Name") {
                ...
                output->setLockPreviousValue(
                    Op::isEq(Op::toScalar(value), 1.0));
    ...
    void FisImporter::importRules(const std::string& section,
                                  Engine* engine) const {
        std::istringstream reader(section);
        std::string line;
        ...
        while (std::getline(reader, line)) {
            ...
            std::vector<std::string> outputsAndRest
                = Op::split(inputsAndRest.at(1), ":");
            ...
            std::vector<std::string> inputs
                = Op::split(inputsAndRest.at(0), " ");
            std::vector<std::string> outputs
                = Op::split(outputsAndRest.at(0), " ");
            ...
            for (std::size_t i = 0; i < inputs.size(); ++i) {
                ...
                ss << engine->getInputVariable(i)->getName() << " "
                   << Rule::isKeyword() << " "
                   << translateProposition(inputCode,
                                           engine->getInputVariable(i));
            ...
            for (std::size_t i = 0; i < outputs.size(); ++i) {
                ...
                ss << engine->getOutputVariable(i)->getName() << " "
                   << Rule::isKeyword() << " "
                   << translateProposition(outputCode,
                                           engine->getOutputVariable(i));
            ...
            for (std::size_t i = 0; i < antecedent.size(); ++i) {
                ...
                if (i + 1 < antecedent.size()) {
                    ...
                    if (connector == "1")
                        ...
                        throw Exception("[syntax error] connector <" + connector
                                            + "> not recognized",
                                        FL_AT);
                }
            }
    ...
    std::string FisImporter::translateProposition(scalar code,
        ...
        if (Op::isEq(fracPart, 0.01))
            ...
            throw Exception(
                "[syntax error] no hedge defined in FIS format for <"
                    + Op::str(fracPart) + ">",
                FL_AT);
        if (not isAny) {
    ...
    std::string FisImporter::translateDefuzzifier(
    ...
    std::pair<scalar, scalar> FisImporter::parseRange(
    ...
    Term* FisImporter::parseTerm(const std::string& fis,
        ...
        if (nameTerm.size() != 2) {
            throw Exception(
                "[syntax error] expected term in format "
                "'name':'class',[params], "
                "but found <"
                    + line + ">",
                FL_AT);
        ...
        return createInstance(Op::trim(termParams.at(0)),
    ...
    Term* FisImporter::createInstance(const std::string& mClass,
        ...
        term.reset(
            FactoryManager::instance()->term()->constructObject(flClass));
