...
namespace fl {
    ...
    Engine* FllImporter::fromString(const std::string& code) const {
        ...
        while (std::getline(fllReader, line)) {
            line = Op::trim(
                Op::split(line, "#", false).front());  // remove comments
            if (line.empty())
                continue;
            std::size_t colon = line.find_first_of(':');
            if (colon == std::string::npos) {
                throw Exception("[import error] expected a colon here: " + line,
                                FL_AT);
            }
            ...
            if ("Engine" == key) {
                ...
                continue;
            } else if (key == "InputVariable" or key == "OutputVariable"
                       or key == "RuleBlock") {
                process(tag, Op::join(block, "\n"), engine.get());
                block.clear();  // clear error flags
                tag = key;
            } else if (tag.empty()) {
                throw Exception("[import error] unexpected block: " + line,
                                FL_AT);
            }
    ...
    void FllImporter::process(const std::string& tag,
        ...
        if ("InputVariable" == tag) {
            ...
            throw Exception(
                "[import error] block tag <" + tag + "> not recognized", FL_AT);
        }
    }

    void FllImporter::processInputVariable(const std::string& block,
                                           Engine* engine) const {
        ...
        while (std::getline(reader, line)) {
            ...
            if ("InputVariable" == keyValue.first) {
                ...
                inputVariable->setLockValueInRange(
                    parseBoolean(keyValue.second));
    ...
    void FllImporter::processOutputVariable(const std::string& block,
                                            Engine* engine) const {
        std::istringstream reader(block);
        std::string line;
        ...
        while (std::getline(reader, line)) {
            ...
            if ("OutputVariable" == keyValue.first) {
                ...
                outputVariable->setDefaultValue(Op::toScalar(keyValue.second));
                       ...
                       or "lock-valid" == keyValue.first) {
                outputVariable->setLockPreviousValue(
                    parseBoolean(keyValue.second));
            } else if ("lock-range" == keyValue.first) {
                outputVariable->setLockValueInRange(
                    parseBoolean(keyValue.second));
            } else if ("defuzzifier" == keyValue.first) {
                outputVariable->setDefuzzifier(
                    parseDefuzzifier(keyValue.second));
            } else if ("aggregation" == keyValue.first) {
                outputVariable->fuzzyOutput()->setAggregation(
                    parseSNorm(keyValue.second));
            } else if ("accumulation" == keyValue.first) {
                outputVariable->fuzzyOutput()->setAggregation(
                    parseSNorm(keyValue.second));
                FL_LOG(
                    "[warning] obsolete usage of identifier <accumulation: "
                    "SNorm> in OutputVariable");
                FL_LOG(
                    "[information] from version 6.0, the identifier "
                    "<aggregation: SNorm> should be used");
                FL_LOG(
    ...
    void FllImporter::processRuleBlock(const std::string& block,
                                       Engine* engine) const {
        std::istringstream reader(block);
        std::string line;
        ...
        while (std::getline(reader, line)) {
            ...
            if ("RuleBlock" == keyValue.first) {
                ...
                if (tnorm->hasConstructor(keyValue.second)) {
                    ruleBlock->setImplication(parseTNorm(keyValue.second));
                    FL_LOG(
                        "[warning] obsolete usage of identifier <activation: "
                        "TNorm> "
                        "in RuleBlock");
                    FL_LOG(
                        "[information] from version 6.0, the identifiers are "
    ...
    Term* FllImporter::parseTerm(const std::string& text,
                                 Engine* engine) const {
        ...
        if (tokens.size() < 2) {
            throw Exception(
                "[syntax error] expected a term in format <name class "
                "parameters>, "
                "but found <"
                    + text + ">",
                FL_AT);
        ...
        term.reset(
            FactoryManager::instance()->term()->constructObject(tokens.at(1)));
    ...
    Activation* FllImporter::parseActivation(const std::string& name) const {
        ...
        Activation* result
            = FactoryManager::instance()->activation()->constructObject(
                tokens.front());
    ...
    Defuzzifier* FllImporter::parseDefuzzifier(const std::string& text) const {
        ...
        Defuzzifier* defuzzifier
            = FactoryManager::instance()->defuzzifier()->constructObject(name);
        if (parameters.size() > 1) {
            ...
            if (IntegralDefuzzifier* integralDefuzzifier
                ...
                integralDefuzzifier->setResolution(
                    (int)Op::toScalar(parameter));
            } else if (WeightedDefuzzifier* weightedDefuzzifier
                       = dynamic_cast<WeightedDefuzzifier*>(defuzzifier)) {
                ...
                if (parameter == "Automatic")
                    ...
                    throw Exception(
                        "[syntax error] unknown parameter of "
                        "WeightedDefuzzifier <"
                            + parameter + ">",
                        FL_AT);
    ...
    std::pair<scalar, scalar> FllImporter::parseRange(
        const std::string& text) const {
        std::pair<std::string, std::string> range = parseKeyValue(text, ' ');
        return std::pair<scalar, scalar>(Op::toScalar(range.first),
                                         Op::toScalar(range.second));
    }
    ...
    std::pair<std::string, std::string> FllImporter::parseKeyValue(
