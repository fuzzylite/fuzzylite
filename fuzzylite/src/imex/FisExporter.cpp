...
std::string FisExporter::exportRules(const Engine* engine) const {
  std::ostringstream fis;
  fis << "[Rules]\n";
  for (std::size_t ixRuleBlock = 0; ixRuleBlock < engine->numberOfRuleBlocks();
       ++ixRuleBlock) {
...
std::string FisExporter::translate(
  ...
  for (std::size_t ixVariable = 0; ixVariable < variables.size();
    ...
    scalar plusHedge = 0;
    int negated = 1;
    for (std::size_t ixProposition = 0; ixProposition < propositions.size();
         ++ixProposition) {
      Proposition* proposition = propositions.at(ixProposition);
      if (proposition->variable != variable)
        continue;

      for (std::size_t termIndex = 0; termIndex < variable->numberOfTerms();
           ++termIndex) {
      ...
      if (hedges.size() > 1) {
        FL_DBG(
            "[exporter warning] only a few combinations of multiple "
...
std::string FisExporter::toString(const Term* term) const {
  ...
  if (const Triangle* x = dynamic_cast<const Triangle*>(term)) {
    ss << "'trimf',["
       << Op::join(3, " ", x->getVertexA(), x->getVertexB(), x->getVertexC())
       << "]";
    return ss.str();
  }
