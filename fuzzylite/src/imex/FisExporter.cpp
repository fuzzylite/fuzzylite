...
namespace fl {
    ...
    std::string FisExporter::exportSystem(const Engine* engine) const {
        ...
        std::string type;
        if (engine->type() == Engine::Mamdani
            or engine->type() == Engine::Larsen) {
            type = "mamdani";
    ...
    std::string FisExporter::exportInputs(const Engine* engine) const {
        std::ostringstream fis;
        for (std::size_t ixVar = 0; ixVar < engine->numberOfInputVariables();
             ++ixVar) {
            InputVariable* var = engine->getInputVariable(ixVar);
            fis << "[Input" << (ixVar + 1) << "]\n";
            fis << "Name='" << Op::validName(var->getName()) << "'\n";
            fis << "Range=["
                << Op::join(2, " ", var->getMinimum(), var->getMaximum())
                << "]\n";
            fis << "NumMFs=" << var->numberOfTerms() << "\n";
            for (std::size_t ixTerm = 0; ixTerm < var->numberOfTerms();
                 ++ixTerm) {
                fis << "MF" << (ixTerm + 1) << "='"
                    << Op::validName(var->getTerm(ixTerm)->getName())
    ...
    std::string FisExporter::exportOutputs(const Engine* engine) const {
        std::ostringstream fis;
        for (std::size_t ixVar = 0; ixVar < engine->numberOfOutputVariables();
             ++ixVar) {
    ...
    std::string FisExporter::exportRules(const Engine* engine) const {
        std::ostringstream fis;
        fis << "[Rules]\n";
        for (std::size_t ixRuleBlock = 0;
             ixRuleBlock < engine->numberOfRuleBlocks();
             ++ixRuleBlock) {
            RuleBlock* rb = engine->getRuleBlock(ixRuleBlock);
            if (engine->numberOfRuleBlocks() > 1)
                fis << "# RuleBlock " << rb->getName() << "\n";
            for (std::size_t ixRule = 0; ixRule < rb->numberOfRules();
                 ++ixRule) {
    ...
    std::string FisExporter::exportRule(const Rule* rule,
        ...
        if (not equalOperators) {
            throw Exception(
                ...
                "(i.e. ['and', 'or']). All connectors within a rule must be "
                "the same",
                FL_AT);
        }
        std::ostringstream fis;
        ...
        if (operators.size() == 0)
            ...
            if (operators.at(0)->name == Rule::andKeyword())
                fis << "1";
    ...
    std::string FisExporter::translate(
        ...
        for (std::size_t ixVariable = 0; ixVariable < variables.size();
             ++ixVariable) {
            Variable* variable = variables.at(ixVariable);
            std::size_t termIndexPlusOne = 0;
            scalar plusHedge = 0;
            int negated = 1;
            for (std::size_t ixProposition = 0;
                 ixProposition < propositions.size();
                 ++ixProposition) {
                ...
                for (std::size_t termIndex = 0;
                     termIndex < variable->numberOfTerms();
                     ++termIndex) {
                ...
                if (hedges.size() > 1) {
                    FL_DBG(
                        "[exporter warning] only a few combinations of "
                        "multiple "
                        "hedges are supported in fis files");
                }
                for (std::size_t ixHedge = 0; ixHedge < hedges.size();
                     ++ixHedge) {
                    ...
                    if (hedge->name() == Not().name())
                        ...
                        plusHedge = fl::nan;  // Unreconized hedge combination
                                              // (e.g. Any)
                }
    ...
    std::string FisExporter::toString(const Term* term) const {
        ...
        if (const Bell* x = dynamic_cast<const Bell*>(term)) {
            ss << "'gbellmf',["
               << Op::join(3, " ", x->getWidth(), x->getSlope(), x->getCenter())
               << "]";
            return ss.str();
        }
        ...
        if (const Binary* x = dynamic_cast<const Binary*>(term)) {
            ss << "'binarymf,["
               << Op::join(2, " ", x->getStart(), x->getDirection()) << "]";
            return ss.str();
        }

        if (const Concave* x = dynamic_cast<const Concave*>(term)) {
            ss << "'concavemf',["
               << Op::join(2, " ", x->getInflection(), x->getEnd()) << "]";
            return ss.str();
        }
        ...
        if (const Cosine* x = dynamic_cast<const Cosine*>(term)) {
            ss << "'cosinemf',["
               << Op::join(2, " ", x->getCenter(), x->getWidth()) << "]";
            return ss.str();
        }
        ...
        if (const Gaussian* x = dynamic_cast<const Gaussian*>(term)) {
            ss << "'gaussmf',["
               << Op::join(2, " ", x->getStandardDeviation(), x->getMean())
               << "]";
            return ss.str();
        }

        if (const GaussianProduct* x
            = dynamic_cast<const GaussianProduct*>(term)) {
            ss << "'gauss2mf',["
               << Op::join(4,
                           " ",
                           x->getStandardDeviationA(),
                           x->getMeanA(),
                           x->getStandardDeviationB(),
                           x->getMeanB())
               << "]";
            return ss.str();
        }
        ...
        if (const Sigmoid* x = dynamic_cast<const Sigmoid*>(term)) {
            ss << "'sigmf',["
               << Op::join(2, " ", x->getSlope(), x->getInflection()) << "]";
            return ss.str();
        }

        if (const SigmoidProduct* x
        ...
        if (const Triangle* x = dynamic_cast<const Triangle*>(term)) {
            ss << "'trimf',["
               << Op::join(
                      3, " ", x->getVertexA(), x->getVertexB(), x->getVertexC())
               << "]";
            return ss.str();
        }
        ...
        ss << "[exporter error] term of class <"
           << (term ? term->className() : "null") << "> not supported";
