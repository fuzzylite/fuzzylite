...
namespace fl {
    ...
    std::string JavaExporter::toString(const InputVariable* inputVariable,
    ...
    std::string JavaExporter::toString(const OutputVariable* outputVariable,
                                       const Engine* engine) const {
        std::ostringstream ss;
        std::string name;
        if (isUsingVariableNames()) {
    ...
    std::string JavaExporter::toString(const RuleBlock* ruleBlock,
                                       const Engine* engine) const {
        std::ostringstream ss;
        std::string name;
        ...
        for (std::size_t i = 0; i < ruleBlock->numberOfRules(); ++i) {
            ...
            ss << name << ".addRule(Rule.parse(\"" << rule->getText()
               << "\", engine));\n";
        }
