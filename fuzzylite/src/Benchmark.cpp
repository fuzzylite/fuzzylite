...
void Benchmark::prepare(int values, FldExporter::ScopeOfValues scope) {
  if (not _engine) {
    throw Exception(
        "[benchmark error] engine not set before preparing for values and "
        "scope",
        FL_AT);
  }
  int resolution;
  if (scope == FldExporter::AllVariables)
    resolution
        = -1
          + (int)std::max(
              1.0, std::pow(values, 1.0 / _engine->numberOfInputVariables()));
  ...
  do {
    ...
    for (std::size_t i = 0; i < _engine->numberOfInputVariables(); ++i) {
      ...
      expectedValues.at(i) = inputVariable->getMinimum()
                             + sampleValues.at(i) * inputVariable->range()
...
std::vector<scalar> Benchmark::run(int times) {
  ...
  for (int t = 0; t < times; ++t) {
    _obtained = std::vector<std::vector<scalar> >(
        _expected.size(), std::vector<scalar>(_engine->variables().size()));
    _engine->restart();
...
std::vector<Benchmark::Result> Benchmark::results(
  ...
  result.push_back(
      Result("mean(t)", Op::str(convert(Op::mean(time), NanoSeconds, unit))));
  result.push_back(
      Result("sd(t)",
