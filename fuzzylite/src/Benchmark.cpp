...
namespace fl {
    ...
    void Benchmark::setExpected(
        const std::vector<std::vector<scalar> >& expected) {
    ...
    void Benchmark::setObtained(
        const std::vector<std::vector<scalar> >& obtained) {
    ...
    void Benchmark::prepare(int values, FldExporter::ScopeOfValues scope) {
        if (not _engine) {
            throw Exception(
                "[benchmark error] engine not set before preparing for values "
                "and scope",
                FL_AT);
        }
        int resolution;
        if (scope == FldExporter::AllVariables)
            resolution = -1
                         + (int)std::max(
                             1.0,
                             std::pow(values,
                                      1.0 / _engine->numberOfInputVariables()));
        ...
        do {
            std::vector<scalar> expectedValues(
                _engine->numberOfInputVariables());
            for (std::size_t i = 0; i < _engine->numberOfInputVariables();
                 ++i) {
    ...
    std::vector<scalar> Benchmark::run(int times) {
        if (not _engine) {
            throw Exception("[benchmark error] engine not set for benchmark",
                            FL_AT);
        }
        ...
        for (int t = 0; t < times; ++t) {
            ...
            for (std::size_t evaluation = 0; evaluation < _expected.size();
                 ++evaluation) {
                const std::vector<scalar>& expectedValues
                    = _expected[evaluation];
                ...
                for (std::size_t i = 0; i < _engine->inputVariables().size();
                     ++i) {
                ...
                for (std::size_t i = 0; i < _engine->outputVariables().size();
                     ++i) {
                    obtainedValues[i + offset]
                        = _engine->getOutputVariable(i)->getValue();
                }
            }
            ...
            runTimes.at(t)
                = std::chrono::duration<scalar, std::nano>(end - start).count();
#endif
        }
    ...
    scalar Benchmark::meanSquaredError(
        ...
        for (std::size_t i = 0; i < _expected.size(); ++i) {
            ...
            for (std::size_t y = 0; y < _engine->numberOfOutputVariables();
                 ++y) {
        ...
        for (std::size_t i = 0; i < _expected.size(); ++i) {
            ...
            for (std::size_t y = 0; y < _engine->numberOfOutputVariables();
                ...
                if (outputVariable == fl::null
                    or outputVariable == _engine->getOutputVariable(y)) {
                    if (not Op::isEq(
                            e.at(y + offset), o.at(y + offset), _tolerance)) {
                        scalar difference = e.at(y + offset) - o.at(y + offset);
                        if (errorType == Accuracy
                            and Op::isFinite(difference)) {
                            ++errors;
                        } else if (errorType == NonFinite
                                   and not Op::isFinite(difference)) {
                            ++errors;
    ...
    std::vector<std::string> Benchmark::header(int runs, bool includeErrors) {
        ...
        if (includeErrors) {
            std::vector<std::vector<scalar> > dummyVector(
                1, std::vector<scalar>(1, fl::nan));
    ...
    std::vector<Benchmark::Result> Benchmark::results(TimeUnit timeUnit,
    ...
    std::vector<Benchmark::Result> Benchmark::results(
        ...
        result.push_back(
            Result("inputs", Op::str(_engine->numberOfInputVariables())));
        result.push_back(
            Result("outputs", Op::str(_engine->numberOfOutputVariables())));
        result.push_back(
            Result("ruleBlocks", Op::str(_engine->numberOfRuleBlocks())));
        std::size_t rules = 0;
        ...
        if (canComputeErrors()) {
            ...
            result.push_back(Result(
                "tolerance",
                Op::str(getTolerance(), -1, std::ios_base::fmtflags(0x0))));
            result.push_back(
                Result("errors", Op::str(allErrors(outputVariable))));

            result.push_back(
                Result("nfErrors", Op::str(nonFiniteErrors(outputVariable))));
            result.push_back(
                Result("accErrors", Op::str(accuracyErrors(outputVariable))));

            result.push_back(
                Result("rmse", Op::str(rmse, 6, std::ios_base::scientific)));
            result.push_back(
                Result("nrmse", Op::str(nrmse, 6, std::ios_base::scientific)));
        }
        result.push_back(Result("units", stringOf(unit)));
        result.push_back(
            Result("sum(t)",
                   Op::str(convert(Op::sum(time), NanoSeconds, unit),
                           unit == NanoSeconds ? 0 : fuzzylite::decimals())));
        result.push_back(Result(
            "mean(t)", Op::str(convert(Op::mean(time), NanoSeconds, unit))));
        result.push_back(Result(
            "sd(t)",
            Op::str(convert(Op::standardDeviation(time), NanoSeconds, unit))));
    ...
    std::string Benchmark::format(std::vector<Result> results,
                                  TableShape shape,
