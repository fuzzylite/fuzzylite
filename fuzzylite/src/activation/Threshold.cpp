...
Threshold::Comparison Threshold::parseComparison(
  ...
  if (name == ">")
    return GreaterThan;
  throw Exception(
      "[syntax error] invalid threshold type by name <" + name + ">", FL_AT);
}
...
Complexity Threshold::complexity(const RuleBlock* ruleBlock) const {
  ...
  for (std::size_t i = 0; i < ruleBlock->rules().size(); ++i) {
    result.comparison(2);
    result += ruleBlock->rules().at(i)->complexity(ruleBlock->getConjunction(),
