...
scalar Rule::activateWith(const TNorm* conjunction, const SNorm* disjunction) {
  if (not isLoaded()) {
    throw Exception(
        "[rule error] the following rule is not loaded: " + getText(), FL_AT);
  }
  _activationDegree
...
bool Rule::isLoaded() const {
  return _antecedent.get() and _consequent.get() and _antecedent->isLoaded()
         and _consequent->isLoaded();
}
...
void Rule::load(const std::string& rule, const Engine* engine) {
  ...
  try {
      ...
      switch (state) {
        ...
        case S_WITH:
          try {
            ...
            std::ostringstream ex;
            ex << "[syntax error] expected a numeric value as the weight of "
                  "the rule: "
               << rule;
    ...
    if (state == S_NONE) {
      ...
      std::ostringstream ex;
      ex << "[syntax error] expected a numeric value as the weight of the "
            "rule: "
         << rule;
      throw Exception(ex.str(), FL_AT);
    }
