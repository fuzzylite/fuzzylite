...
namespace fl {
    ...
    scalar Rule::activateWith(const TNorm* conjunction,
                              const SNorm* disjunction) {
        if (not isLoaded()) {
            throw Exception(
                "[rule error] the following rule is not loaded: " + getText(),
                FL_AT);
        }
        _activationDegree
            = _weight * _antecedent->activationDegree(conjunction, disjunction);
    ...
    void Rule::trigger(const TNorm* implication) {
        ...
        if (_enabled and Op::isGt(_activationDegree, 0.0)) {
            FL_DBG("[firing with " << Op::str(_activationDegree) << "] "
                                   << toString());
    ...
    Complexity Rule::complexityOfActivation(const TNorm* conjunction,
    ...
    Complexity Rule::complexity(const TNorm* conjunction,
                                const SNorm* disjunction,
    ...
    void Rule::load(const std::string& rule, const Engine* engine) {
        ...
        try {
                ...
                switch (state) {
                    case S_NONE:
                        if (token == Rule::ifKeyword())
                            ...
                            ex << "[syntax error] expected keyword <"
                               << Rule::ifKeyword() << ">, but found <" << token
                               << "> in rule: " << rule;
                    ...
                    case S_WITH:
                        try {
                            ...
                            std::ostringstream ex;
                            ex << "[syntax error] expected a numeric value as "
            ...
            if (state == S_NONE) {
                std::ostringstream ex;
                ex << "[syntax error] "
                   << (rule.empty() ? "empty rule" : ("ignored rule: " + rule));
                throw Exception(ex.str(), FL_AT);
            } else if (state == S_IF) {
                std::ostringstream ex;
                ex << "[syntax error] keyword <" << Rule::thenKeyword()
                   << "> not found in rule: " << rule;
                throw Exception(ex.str(), FL_AT);
            } else if (state == S_WITH) {
                std::ostringstream ex;
                ex << "[syntax error] expected a numeric value as the weight "
                      "of the rule: "
                   << rule;
                throw Exception(ex.str(), FL_AT);
            }
