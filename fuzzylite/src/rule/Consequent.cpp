...
void Consequent::modify(scalar activationDegree, const TNorm* implication) {
  if (not isLoaded()) {
    throw Exception(
        "[consequent error] consequent <" + getText() + "> is not loaded",
        FL_AT);
  }
  for (std::size_t i = 0; i < _conclusions.size(); ++i) {
    Proposition* proposition = _conclusions.at(i);
  ...
  try {
    while (tokenizer >> token) {
      ...
      if (state bitand S_VARIABLE) {
        ...
        ex << "[syntax error] consequent expected output variable, but found <"
           << token << ">";
      ...
      if ((state bitand S_HEDGE) or (state bitand S_TERM)) {
        std::ostringstream ex;
        ex << "[syntax error] consequent expected hedge or term, but found <"
           << token << ">";
        throw Exception(ex.str(), FL_AT);
      }
    ...
    if (not((state bitand S_AND)
      ...
      if (state bitand S_VARIABLE) {
        std::ostringstream ex;
        ex << "[syntax error] consequent expected output variable after <"
           << token << ">";
        throw Exception(ex.str(), FL_AT);
      }
      if (state bitand S_IS) {
      ...
      if ((state bitand S_HEDGE) or (state bitand S_TERM)) {
        std::ostringstream ex;
        ex << "[syntax error] consequent expected hedge or term after <"
           << token << ">";
        throw Exception(ex.str(), FL_AT);
      }
    }
