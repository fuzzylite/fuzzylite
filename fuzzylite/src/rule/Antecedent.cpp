...
scalar Antecedent::activationDegree(const TNorm* conjunction,
  ...
  if (not isLoaded()) {
    throw Exception(
        "[antecedent error] antecedent <" + getText() + "> is not loaded",
        FL_AT);
  }
  ...
  try {
    while (tokenizer >> token) {
      ...
      if ((state bitand S_VARIABLE) or (state bitand S_AND_OR)) {
        std::ostringstream ex;
        ex << "[syntax error] antecedent expected variable or logical "
              "operator, but found <"
           << token << ">";
        throw Exception(ex.str(), FL_AT);
      }
      ...
      if ((state bitand S_HEDGE) or (state bitand S_TERM)) {
        std::ostringstream ex;
        ex << "[syntax error] antecedent expected hedge or term, but found <"
           << token << ">";
        throw Exception(ex.str(), FL_AT);
      }
      std::ostringstream ex;
    ...
    if (not((state bitand S_VARIABLE)
            or (state bitand S_AND_OR))) {  // only acceptable final state
      if (state bitand S_IS) {
        std::ostringstream ex;
        ex << "[syntax error] antecedent expected keyword <"
      ...
      if ((state bitand S_HEDGE) or (state bitand S_TERM)) {
        std::ostringstream ex;
        ex << "[syntax error] antecedent expected hedge or term after <"
           << token << ">";
        throw Exception(ex.str(), FL_AT);
      }
    }
    ...
    if (expressionStack.size() != 1) {
      ...
      std::ostringstream ex;
      ex << "[syntax error] unable to parse the following expressions in "
            "antecedent <"
         << Op::join(errors, " ") << ">";
      throw Exception(ex.str(), FL_AT);
    }
...
std::string Antecedent::toPrefix(const Expression* node) const {
  if (not isLoaded()) {
    throw Exception(
        "[antecedent error] antecedent <" + _text + "> is not loaded", FL_AT);
  }
  if (not node)
    node = getExpression();
...
std::string Antecedent::toPostfix(const Expression* node) const {
  ...
  if (const Operator* fuzzyOperator = dynamic_cast<const Operator*>(node)) {
    ss << toPostfix(fuzzyOperator->left) << " "
       << toPostfix(fuzzyOperator->right) << " " << fuzzyOperator->toString()
       << " ";
  } else {
    ss << "[antecedent error] unknown class of Expression <"
       << (node ? node->toString() : "null") << ">";
