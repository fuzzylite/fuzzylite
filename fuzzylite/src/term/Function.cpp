...
namespace fl {
    ...
    scalar Function::Node::evaluate(
        const std::map<std::string, scalar>* variables) const {
        scalar result = fl::nan;
        if (element.get()) {
            if (element->unary) {
                result = element->unary(left->evaluate(variables));
            } else if (element->binary) {
                result = element->binary(right->evaluate(variables),
            ...
            std::map<std::string, scalar>::const_iterator it
    ...
    std::size_t Function::Node::treeSize(Element::Type type,
    ...
    Complexity Function::complexity() const {
        ...
        if (_engine) {             // insert variables in map
            ...
            result.function(
                engineVariables
                * std::log(scalar(variables.size() + engineVariables)));
            result.function(
                1 * std::log(scalar(variables.size() + engineVariables)));
        }
        if (_root.get()) {
            ...
            result.function(treeSize
                            * std::log(treeSize));  // only operands in tree
        }
        return result;
    }
    ...
    scalar Function::membership(scalar x) const {
        if (not _root.get()) {
            throw Exception(
                "[function error] function <" + _formula + "> not loaded.",
                FL_AT);
        }
        if (_engine) {
            ...
            for (std::size_t i = 0; i < _engine->numberOfOutputVariables();
                 ++i) {
    ...
    scalar Function::evaluate(
        const std::map<std::string, scalar>* localVariables) const {
        if (not _root.get())
            throw Exception(
                "[function error] evaluation failed because the function is "
                "not loaded",
                FL_AT);
        if (localVariables)
    ...
    void Function::load(const std::string& formula, const Engine* engine) {
        ...
        membership(
            0.0);  // make sure function evaluates without throwing exception.
    }
    ...
    std::string Function::space(const std::string& formula) const {
        ...
        std::vector<std::string> operators
            = FactoryManager::instance()->function()->availableOperators();
        for (std::size_t i = 0; i < operators.size(); ++i) {
            if (not(operators.at(i) == Rule::andKeyword()
                    or operators.at(i) == Rule::orKeyword())) {
        ...
        for (std::size_t i = 0; i < chars.size(); ++i) {
            result
                = Op::findReplace(result, chars.at(i), " " + chars.at(i) + " ");
        }
        return result;
    }
    ...
    std::string Function::toPostfix(const std::string& formula) const {
        ...
        while (tokenizer >> token) {
            ...
            bool isOperand = not element and token != "(" and token != ")"
                             and token != ",";
            ...
            if (isOperand) {
                ...
                if (stack.empty() or stack.top() != "(") {
                    std::ostringstream ex;
                    ex << "[parsing error] mismatching parentheses in: "
                       << formula;
                    throw Exception(ex.str(), FL_AT);
                }
                ...
                for (;;) {
                    ...
                    if ((op1->associativity < 0
                         and op1->precedence == op2->precedence)
    ...
    Function::Node* Function::parse(const std::string& formula) {
        ...
        if (stack.size() != 1)
            throw Exception(
                "[function error] ill-formed formula <" + formula + ">", FL_AT);
