...
Complexity Function::complexity() const {
  ...
  if (_engine) {             // insert variables in map
    ...
    result.function(engineVariables
...
scalar Function::membership(scalar x) const {
  if (not _root.get()) {
    throw Exception("[function error] function <" + _formula + "> not loaded.",
                    FL_AT);
  }
  if (_engine) {
...
scalar Function::evaluate(
  ...
  if (not _root.get())
    throw Exception(
        "[function error] evaluation failed because the function is not loaded",
        FL_AT);
  if (localVariables)
...
std::string Function::toPostfix(const std::string& formula) const {
  ...
  while (tokenizer >> token) {
    ...
    bool isOperand
        = not element and token != "(" and token != ")" and token != ",";
...
Function::Node* Function::parse(const std::string& formula) {
  ...
  if (stack.size() != 1)
    throw Exception("[function error] ill-formed formula <" + formula + ">",
                    FL_AT);
