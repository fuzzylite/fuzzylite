    ...
    if (not ruleblock) {
      ...
      for (std::size_t r = 0; r < ruleblock->numberOfRules(); ++r) {
        ...
        if (not rule) {
          ...
          std::size_t thenIndex
              = rule->getText().find(" " + Rule::thenKeyword() + " ");
          std::size_t andIndex
              = rule->getText().find(" " + Rule::andKeyword() + " ");
          std::size_t orIndex
              = rule->getText().find(" " + Rule::orKeyword() + " ");
...
void Engine::process() {
  ...
  for (std::size_t i = 0; i < _outputVariables.size(); ++i) {
    ...
    if (outputVariable->isEnabled()) {
      ...
      FL_DBG(outputVariable->getName()
             << ".lockValueInRange = " << outputVariable->isLockValueInRange());
...
Engine::Type Engine::type(std::string* name, std::string* reason) const {
  ...
  for (std::size_t i = 0; takagiSugeno and i < outputVariables().size(); ++i) {
    ...
    takagiSugeno
        = takagiSugeno and weightedDefuzzifier
          and (weightedDefuzzifier->getType() == WeightedDefuzzifier::Automatic
  ...
  for (std::size_t i = 0; tsukamoto and i < outputVariables().size(); ++i) {
    ...
    if (tsukamoto) {
      // Tsukamoto has only monotonic terms: Concave, Ramp, Sigmoid, SShape, or
      // ZShape
      for (std::size_t t = 0; tsukamoto and t < outputVariable->numberOfTerms();
           ++t) {
        Term* term = outputVariable->getTerm(t);
  ...
  for (std::size_t i = 0; inverseTsukamoto and i < outputVariables().size();
       ++i) {
    OutputVariable* outputVariable = outputVariables().at(i);
  ...
  if (inverseTsukamoto) {
    ...
    if (reason)
      *reason
          = "- Output variables have weighted defuzzifiers\n"
            "- Output variables do not only have constant, linear or function "
            "terms\n"
...
InputVariable* Engine::getInputVariable(const std::string& name) const {
  ...
  throw Exception("[engine error] input variable <" + name + "> not found",
                  FL_AT);
}
...
OutputVariable* Engine::getOutputVariable(const std::string& name) const {
  ...
  throw Exception("[engine error] output variable <" + name + "> not found",
                  FL_AT);
}
