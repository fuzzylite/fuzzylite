...
namespace fl {
    ...
    void Engine::copyFrom(const Engine& other) {
        ...
        for (std::size_t i = 0; i < other._inputVariables.size(); ++i)
            _inputVariables.push_back(
                new InputVariable(*other._inputVariables.at(i)));
        for (std::size_t i = 0; i < other._outputVariables.size(); ++i)
            _outputVariables.push_back(
                new OutputVariable(*other._outputVariables.at(i)));
    ...
    void Engine::configure(const std::string& conjunction,
        ...
        DefuzzifierFactory* defuzzFactory
            = FactoryManager::instance()->defuzzifier();
        ActivationFactory* activationFactory
            = FactoryManager::instance()->activation();
        ...
        Defuzzifier* defuzzifierObject
            = defuzzFactory->constructObject(defuzzifier);
        Activation* activationObject
            = activationFactory->constructObject(activation);

        configure(conjunctionObject,
                  disjunctionObject,
                  implicationObject,
                  aggregationObject,
                  defuzzifierObject,
    ...
    void Engine::configure(TNorm* conjunction,
                           SNorm* disjunction,
                           TNorm* implication,
                           SNorm* aggregation,
        ...
        for (std::size_t i = 0; i < numberOfRuleBlocks(); ++i) {
            ...
            ruleBlock->setConjunction(conjunction ? conjunction->clone()
                                                  : fl::null);
            ruleBlock->setDisjunction(disjunction ? disjunction->clone()
                                                  : fl::null);
            ruleBlock->setImplication(implication ? implication->clone()
                                                  : fl::null);
            ruleBlock->setActivation(activation ? activation->clone()
                                                : new General);
        }

        for (std::size_t i = 0; i < numberOfOutputVariables(); ++i) {
            OutputVariable* outputVariable = getOutputVariable(i);
            outputVariable->setDefuzzifier(defuzzifier ? defuzzifier->clone()
                                                       : fl::null);
            outputVariable->setAggregation(aggregation ? aggregation->clone()
                                                       : fl::null);
        }
        if (defuzzifier)
            delete defuzzifier;
    ...
    bool Engine::isReady(std::string* status) const {
        ...
        for (std::size_t i = 0; i < inputVariables().size(); ++i) {
            ...
            if (not inputVariable) {
                ss << "- Engine <" << getName()
                   << "> has a fl::null input variable at index <" << i
                   << ">\n";
            }
            /*else if (inputVariable->terms().empty()) {
            ignore because sometimes inputs can be empty:
            takagi-sugeno/matlab/slcpp1.fis ss << "- Input variable <" <<
            _inputVariables.at(i)->getName() << ">"
                                    << " has no terms\n";
            }*/
        }
        ...
        for (std::size_t i = 0; i < outputVariables().size(); ++i) {
            ...
            if (not outputVariable) {
                ss << "- Engine <" << getName()
                   << "> has a fl::null output variable at index <" << i
                   << ">\n";
            } else {
                ...
                SNorm* aggregation
                    = outputVariable->fuzzyOutput()->getAggregation();
                if (not aggregation
                    and dynamic_cast<IntegralDefuzzifier*>(defuzzifier)) {
                    ss << "- Output variable <" << outputVariable->getName()
                       << ">"
        ...
        for (std::size_t i = 0; i < ruleBlocks().size(); ++i) {
            ...
            if (not ruleblock) {
                ss << "- Engine <" << getName()
                   << "> has a fl::null rule block at index <" << i << ">\n";
            } else {
                if (ruleblock->rules().empty()) {
                    ss << "- Rule block " << (i + 1) << " <"
                       << ruleblock->getName() << "> has no rules\n";
                }
                ...
                for (std::size_t r = 0; r < ruleblock->numberOfRules(); ++r) {
                    ...
                    if (not rule) {
                        ...
                        std::size_t thenIndex = rule->getText().find(
                            " " + Rule::thenKeyword() + " ");
                        std::size_t andIndex = rule->getText().find(
                            " " + Rule::andKeyword() + " ");
                        std::size_t orIndex = rule->getText().find(
                            " " + Rule::orKeyword() + " ");
                        if (andIndex != std::string::npos
                            and andIndex < thenIndex) {
                            ++requiresConjunction;
                        }
                        if (orIndex != std::string::npos
                            and orIndex < thenIndex) {
                            ++requiresDisjunction;
                        }
                        if (rule->isLoaded()) {
                            Consequent* consequent = rule->getConsequent();
                            for (std::size_t c = 0;
                                 c < consequent->conclusions().size();
                                 ++c) {
                                Proposition* proposition
                                    = consequent->conclusions().at(c);
                                const OutputVariable* outputVariable
                                    = dynamic_cast<const OutputVariable*>(
                                        proposition->variable);
                                if (outputVariable
                                    and dynamic_cast<IntegralDefuzzifier*>(
    ...
    void Engine::process() {
        ...
        for (std::size_t i = 0; i < _inputVariables.size(); ++i) {
            ...
            if (inputVariable->isEnabled()) {
                FL_DBG(inputVariable->getName()
                       << ".input = " << Op::str(inputValue));
                FL_DBG(inputVariable->getName()
                       << ".fuzzy = " << inputVariable->fuzzify(inputValue));
            } else {
        ...
        for (std::size_t i = 0; i < _outputVariables.size(); ++i) {
            ...
            if (outputVariable->isEnabled()) {
                FL_DBG(outputVariable->getName()
                       << ".default = " << outputVariable->getDefaultValue());
                ...
                FL_DBG(outputVariable->getName()
                       << ".fuzzy = " << outputVariable->fuzzify(output));
    ...
    Engine::Type Engine::type(std::string* name, std::string* reason) const {
        ...
        if (mamdani) {
            for (std::size_t i = 0; larsen and i < ruleBlocks().size(); ++i) {
                RuleBlock* ruleBlock = ruleBlocks().at(i);
                larsen = larsen
                         and dynamic_cast<const AlgebraicProduct*>(
            ...
            if (reason)
                *reason
                    = "- Output variables have integral defuzzifiers\n"
                      "- Implication in rule blocks is the algebraic product "
                      "T-Norm";
        ...
        for (std::size_t i = 0; takagiSugeno and i < outputVariables().size();
             ++i) {
            OutputVariable* outputVariable = outputVariables().at(i);
            // Defuzzifier is Weighted
            WeightedDefuzzifier* weightedDefuzzifier
                = dynamic_cast<WeightedDefuzzifier*>(
                    outputVariable->getDefuzzifier());

            takagiSugeno = takagiSugeno and weightedDefuzzifier
                           and (weightedDefuzzifier->getType()
                                    == WeightedDefuzzifier::Automatic
                                or weightedDefuzzifier->getType()
                                       == WeightedDefuzzifier::TakagiSugeno);

            if (takagiSugeno) {
                // Takagi-Sugeno has only Constant, Linear or Function terms
                for (std::size_t t = 0;
                     takagiSugeno and t < outputVariable->numberOfTerms();
                     ++t) {
                    Term* term = outputVariable->getTerm(t);
                    takagiSugeno = takagiSugeno
                                   and weightedDefuzzifier->inferType(term)
                                           == WeightedDefuzzifier::TakagiSugeno;
                }
            }
        }
            ...
            if (reason)
                *reason
                    = "- Output variables have weighted defuzzifiers\n"
                      "- Output variables have constant, linear or function "
                      "terms";
        ...
        // Tsukamoto
        bool tsukamoto = true;
        for (std::size_t i = 0; tsukamoto and i < outputVariables().size();
             ++i) {
            OutputVariable* outputVariable = outputVariables().at(i);
            // Defuzzifier is Weighted
            WeightedDefuzzifier* weightedDefuzzifier
            ...
            tsukamoto = tsukamoto and weightedDefuzzifier
                        and (weightedDefuzzifier->getType()
                                 == WeightedDefuzzifier::Automatic
                             or weightedDefuzzifier->getType()
                                    == WeightedDefuzzifier::Tsukamoto);
            if (tsukamoto) {
                // Tsukamoto has only monotonic terms: Concave, Ramp, Sigmoid,
                // SShape, or ZShape
                for (std::size_t t = 0;
                     tsukamoto and t < outputVariable->numberOfTerms();
                     ++t) {
                    Term* term = outputVariable->getTerm(t);
        ...
        for (std::size_t i = 0;
             inverseTsukamoto and i < outputVariables().size();
             ++i) {
            OutputVariable* outputVariable = outputVariables().at(i);
        ...
        if (inverseTsukamoto) {
            ...
            if (reason)
                *reason
                    = "- Output variables have weighted defuzzifiers\n"
                      "- Output variables do not only have constant, linear or "
                      "function terms\n"
    ...
    std::vector<Variable*> Engine::variables() const {
        ...
        result.insert(
            result.end(), inputVariables().begin(), inputVariables().end());
        result.insert(
            result.end(), outputVariables().begin(), outputVariables().end());
        return result;
    }
    ...
    InputVariable* Engine::setInputVariable(InputVariable* inputVariable,
                                            std::size_t index) {
    ...
    void Engine::insertInputVariable(InputVariable* inputVariable,
                                     std::size_t index) {
        inputVariables().insert(inputVariables().begin() + index,
                                inputVariable);
    }
    ...
    InputVariable* Engine::getInputVariable(const std::string& name) const {
        ...
        throw Exception(
            "[engine error] input variable <" + name + "> not found", FL_AT);
    }
    ...
    OutputVariable* Engine::setOutputVariable(OutputVariable* outputVariable,
                                              std::size_t index) {
    ...
    void Engine::insertOutputVariable(OutputVariable* outputVariable,
                                      std::size_t index) {
        outputVariables().insert(outputVariables().begin() + index,
                                 outputVariable);
    }
    ...
    OutputVariable* Engine::getOutputVariable(const std::string& name) const {
        ...
        throw Exception(
            "[engine error] output variable <" + name + "> not found", FL_AT);
    }
    ...
    void Engine::setOutputVariables(
    ...
    RuleBlock* Engine::getRuleBlock(const std::string& name) const {
        ...
        throw Exception("[engine error] rule block <" + name + "> not found",
                        FL_AT);
    }
