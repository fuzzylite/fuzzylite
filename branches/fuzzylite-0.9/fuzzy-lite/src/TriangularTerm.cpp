...
namespace fl {
	...
	flScalar TriangularTerm::membership(flScalar crisp) const {
		...
		flScalar result = crisp < b() ? FuzzyOperator::Scale(a(), b(), crisp, 0, 1)
			: FuzzyOperator::Scale(b(), c(), crisp, 1, 0);
		return fuzzyOperator().modulate(result, modulation());
	}
