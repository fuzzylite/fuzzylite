...
namespace fl {
	...
	flScalar ShoulderTerm::membership(flScalar crisp) const {
		if (isLeft()) {
			...
			if (crisp > maximum())
				return flScalar(0.0);
			return FuzzyOperator::Scale(minimum(), maximum(), crisp, 1, 0);
		} else {
			if (crisp < minimum())
				return flScalar(0.0);
			if (crisp > maximum())
				return flScalar(1.0);
			return FuzzyOperator::Scale(minimum(), maximum(), crisp, 0, 1);
		}
	}
	...
	std::string ShoulderTerm::toString() const {
		std::stringstream ss;
		ss << name() << "={ min:" << minimum() << ", max:" << maximum() << ", dir:"
			<< (isLeft() ? "left" : "right") << ", type:" << type() << " }";
		return ss.str();
	}
