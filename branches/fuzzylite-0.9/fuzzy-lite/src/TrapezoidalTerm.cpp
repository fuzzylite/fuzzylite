...
namespace fl {
	...
	TrapezoidalTerm::TrapezoidalTerm(const std::string& name, flScalar minimum,
		flScalar maximum) :
		LinguisticTerm(name, minimum, maximum), _b((maximum - minimum) * 1 / 5), _c(
			(maximum - minimum) * 4 / 5) {
	...
	TrapezoidalTerm::TrapezoidalTerm(const FuzzyOperator& fuzzy_op,
		...
		LinguisticTerm(fuzzy_op, name, minimum, maximum),
			_b((maximum - minimum) * 1 / 5), _c((maximum - minimum) * 4 / 5) {
	...
	flScalar TrapezoidalTerm::membership(flScalar crisp) const {
		...
		if (crisp < b()) {
			return fuzzyOperator().modulate(modulation(), FuzzyOperator::Scale(a(), b(),
				crisp, 0, 1));
		}
		if (crisp < c()) {
			return fuzzyOperator().modulate(modulation(), flScalar(1.0));
		}
		if (crisp < d()) {
			return fuzzyOperator().modulate(modulation(), FuzzyOperator::Scale(c(), d(),
				crisp, 1, 0));
		}
		return flScalar(0.0);
	}
