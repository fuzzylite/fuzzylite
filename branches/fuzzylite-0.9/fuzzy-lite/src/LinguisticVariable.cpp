...
namespace fl {
	...
	int LinguisticVariable::positionFor(const LinguisticTerm& term) {
		for (size_t i = 0; i < _terms.size(); ++i) {
			if (FuzzyOperator::IsLEq(term.minimum(), _terms[i]->minimum())) {
				if (FuzzyOperator::IsEq(term.minimum(), _terms[i]->minimum())) {
					return term.membership(term.minimum()) > _terms[i]->membership(
						term.minimum()) ? i : i + 1;
				}
				return i;
			}
	...
	void LinguisticVariable::addTerm(LinguisticTerm& term) {
		_terms.insert(_terms.begin() + positionFor(term), &term);
	}

	LinguisticTerm* LinguisticVariable::term(int index) const throw (OutOfRangeException) {
	...
	LinguisticTerm* LinguisticVariable::removeTerm(int index) throw (OutOfRangeException) {
		OutOfRangeException::CheckArray(FL_AT,index, numberOfTerms()) ;
	...
	LinguisticTerm* LinguisticVariable::removeTerm(const std::string& name) throw (InvalidArgumentException) {
		int index = indexOf(name);
		if (index == -1){
			throw InvalidArgumentException(FL_AT, "Term <" + name + "> not registered in " + this->name());
		}
		return removeTerm(index);
	}

	void LinguisticVariable::deleteTerm(int index) throw (OutOfRangeException){
		delete removeTerm(index);
	}

	void LinguisticVariable::deleteTerm(const std::string& name) throw (InvalidArgumentException){
		delete removeTerm(name);
	}
	...
	std::string LinguisticVariable::fuzzify(flScalar crisp) const {
		...
		for (int i = 0; i < numberOfTerms(); ++i) {
			...
			fuzzyness.push_back(StringOperator::ScalarToString(degree) + "/"
				+ term(i)->name());
		}
