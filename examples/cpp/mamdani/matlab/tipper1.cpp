...
ruleblock1->addRule(fl::FuzzyRule::parse("if service is poor then tip is cheap", engine));
ruleblock1->addRule(fl::FuzzyRule::parse("if service is good then tip is average", engine));
ruleblock1->addRule(fl::FuzzyRule::parse("if service is excellent then tip is generous", engine));
