...
ruleblock1->addRule(fl::FuzzyRule::parse("if angle is small and velocity is small then force is negBig and force2 is posBig2", engine));
ruleblock1->addRule(fl::FuzzyRule::parse("if angle is small and velocity is big then force is negSmall and force2 is posSmall2", engine));
ruleblock1->addRule(fl::FuzzyRule::parse("if angle is big and velocity is small then force is posSmall and force2 is negSmall2", engine));
ruleblock1->addRule(fl::FuzzyRule::parse("if angle is big and velocity is big then force is posBig and force2 is negBig2", engine));
