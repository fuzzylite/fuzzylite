...
ruleblock1->addRule(fl::FuzzyRule::parse("if angle is small and velocity is small then force is negBig", engine));
ruleblock1->addRule(fl::FuzzyRule::parse("if angle is small and velocity is big then force is negSmall", engine));
ruleblock1->addRule(fl::FuzzyRule::parse("if angle is big and velocity is small then force is posSmall", engine));
ruleblock1->addRule(fl::FuzzyRule::parse("if angle is big and velocity is big then force is posBig", engine));
